...
require_once $CFG->dirroot.'/grade/report/audit/lib.php';
...
$PAGE->set_url(new moodle_url('/grade/report/audit/index.php', array('id'=>$courseid)));
...
require_capability('gradereport/audit:view', $context);
require_capability('moodle/grade:viewall', $context);

/// return tracking object
$gpr = new grade_plugin_return(array('type'=>'report', 'plugin'=>'audit', 'courseid'=>$courseid, 'page'=>$page));

/// last selected report session tracking
if (!isset($USER->grade_last_report)) {
    $USER->grade_last_report = array();
}
$USER->grade_last_report[$course->id] = 'audit';
...
if (!empty($target) && !empty($action) && confirm_sesskey()) {
    grade_report_audit::process_action($target, $action);
}

$reportname = get_string('pluginname', 'gradereport_audit');

/// Print header
print_grade_page_head($COURSE->id, 'report', 'audit', $reportname, false, $buttons);

//Initialise the audit report object that produces the table
//the class grade_report_grader_ajax was removed as part of MDL-21562
$report = new grade_report_audit($courseid, $gpr, $context, $page, $sortitemid);
