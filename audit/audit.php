...
require_once $CFG->dirroot.'/grade/report/audit/auditlib.php';
...
$url = new moodle_url('/grade/report/audit/audit.php', array('courseid'=>$courseid));
if ($id !== 0) {
	$url->param('id', $id);
}
if ($itemid !== 0) {
...
// Start the page.
/// Print header
$reportname = get_string('grade_audit', 'gradereport_audit'); // ... which it isn't, but that will do for now.
print_grade_page_head($COURSE->id, 'report', 'audit', $reportname, false, '');
...
try {
	...
	print_error('queryfailed', 'gradereport_audit', grade_audit_url('index.php'),
			$e->getMessage());
}

// output some details about this report...
$course_details = get_string('grade_audit_course', 'gradereport_audit').$courseid;
$user_details = get_string('grade_audit_user', 'gradereport_audit').$userid;
$item_details = get_string('grade_audit_item', 'gradereport_audit').$itemid;
$sql_query = get_string('grade_audit_query', 'gradereport_audit').$report;
...
if (is_null($csvfilepath)) {
    echo html_writer::tag('p', get_string('noaudittrail', 'gradereport_audit'));
} else {
    ...
    if (!is_readable($csvfilepath)) {
        echo html_writer::tag('p', get_string('filereaderror', 'gradereport_audit'). $csvfilepath);
    } else {
        ...
        if ($count >= GRADE_AUDIT_MAX_RECORDS) {
            echo html_writer::tag('p', get_string('recordlimitreached', 'gradereport_audit',
                                                  GRADE_AUDIT_MAX_RECORDS),
                                                  array('class' => 'admin_note'));
        }
        echo html_writer::start_tag('p').
             html_writer::tag('a', get_string('downloadthisreportascsv', 'gradereport_audit'),
...
echo html_writer::start_tag('p').
     $OUTPUT->action_link(new moodle_url(grade_audit_url('index.php?id='.$courseid)), $imglarrow.
                                         get_string('backtoauditreport', 'gradereport_audit')).
     html_writer::end_tag('p').
     $OUTPUT->footer();
