...
/**
 * File in which the audit_report class is defined.
 * @package gradebook
 */
...
/**
 * Class providing an API for the audit report building and displaying.
 * @uses grade_report
 * @package gradebook
 */
class grade_report_audit extends grade_report {
    /**
     * The final grades.
     * @var array $grades
     */
...
//// SQL-RELATED
    ...
    public function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null) {
        ...
        if ($collapsed = get_user_preferences('grade_report_audit_collapsed_categories')) {
        ...
        $this->pbarurl = new moodle_url('/grade/report/audit/index.php', array('id' => $this->courseid, 'perpage' => $studentsperpage));
...
/**
    ...
    /**
    * Shortcut function for printing the audit report toggles.
    ...
    */
    ...
    /**
     * Builds and returns the rows that will make up the left part of the audit report
    ...
    /**
     * Builds and returns the rows that will make up the right part of the audit report
     * @return array Array of html_table_row objects
     */
...
// Element is a grade_item
        ...
        foreach ($this->users as $userid => $user) {
            ...
            foreach ($this->gtree->items as $itemid=>$unused) {
                ...
                // MDL-11274
                // Hide grades in the audit report if the current audit doesn't have 'moodle/grade:viewhidden'
        ...
        $module = array(
            'name'      => 'gradereport_audit',
            'fullpath'  => '/grade/report/audit/module.js',
            'requires'  => array('base', 'dom', 'event', 'event-mouseenter', 'event-key', 'io-base', 'json-parse', 'overlay')
        );
        $PAGE->requires->js_init_call('M.gradereport_audit.init_report', $jsarguments, false, $module);
        $PAGE->requires->strings_for_js(array('addfeedback','feedback', 'grade'), 'grades');
        $PAGE->requires->strings_for_js(array('ajaxchoosescale','ajaxclicktoclose','ajaxerror','ajaxfailedupdate', 'ajaxfieldchanged'), 'gradereport_audit');
    ...
    /**
     * Builds and return the header for the row of ranges, for the left part of the audit report.
     * @param array $rows The Array of rows for the left part of the report
     * @param int $colspan The number of columns this cell has to span
     * @return array Array of rows for the left part of the report
     */
    ...
    /**
     * Builds and return the headers for the rows of averages, for the left part of the audit report.
     * @param array $rows The Array of rows for the left part of the report
     * @param int $colspan The number of columns this cell has to span
    ...
    /**
     * Builds and return the row of icons when editing is on, for the right part of the audit report.
    ...
    /**
     * Builds and return the row of ranges for the right part of the audit report.
     * @param array $rows The Array of rows for the right part of the report
     * @return array Array of rows for the right part of the report
     */
    ...
    /**
     * Builds and return the row of averages for the right part of the audit report.
    ...
    protected function get_audit_icon($element, $gpr) {
    	...
    	if (is_null($strgradeaudit)) {
    		$strgradeaudit = get_string('grade_audit', 'gradereport_audit');
    	}
     ...
     * with the icons needed for the audit report.
     *
     * @param array $object
     * @return string HTML
     */
    protected function get_icons($element) {
        ...
        return $OUTPUT->container($editicon.$editcalculationicon.$showhideicon.$lockunlockicon.$gradeanalysisicon.$gradeauditicon, 'grade_icons');
    }
        ...
        switch ($action) {
            case 'switch_minus': // Add category to array of aggregatesonly
                if (!in_array($targetid, $collapsed['aggregatesonly'])) {
                    ...
                    set_user_preference('grade_report_audit_collapsed_categories', serialize($collapsed));
                }
                break;
